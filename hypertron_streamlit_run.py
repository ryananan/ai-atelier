...
def video_generation(args, DefaultPaths,status,filename):
    ...
    # You can change i to the number of the last frame you want to generate. It will raise an error if that number of frames does not exist.
    last_frame = how_many_frames

    min_fps = 10
    max_fps = how_many_fps

    total_frames = last_frame-init_frame
    length = 3  # Desired time of the video in seconds
    ...
    for i in range(init_frame, last_frame):
        imageName = "%05d" % i
        frames.append(Image.open(intermediary_folder + "/FRA" + str(imageName) + '.PNG'))
    ...
    p = Popen(['ffmpeg', '-y', '-f', 'image2pipe', '-vcodec', 'png', '-r',
               str(fps), '-i', '-', '-vcodec', 'libx264', '-r',
               str(fps), '-pix_fmt', 'yuv420p', '-crf', '17', '-preset', 'veryslow',
               filename+'.mp4'], stdin=PIPE)

 
    bar_container = status.container()
    my_progress_bar = bar_container.progress(0)
    my_iteration_counter = bar_container.empty()
    percent_complete = 0
    ...
    try:
        ...
        status.write("🥳 Video generated 🥳")

        # move video files
        import shutil
        old_path = '/content/'+filename+'.mp4'
...
def run_model(args2, status, stoutput, DefaultPaths):
    ...
    filename_prompt = sanitize_filename(args2.prompt)
    
    if len(args2.prompt) > 60:
        l = len(args2.prompt)
        l2 = l-60
        filename_prompt = args2.prompt[:l-l2]
        filename_prompt = sanitize_filename(filename_prompt)
    print("New shorten prompts in filename:" + filename_prompt)

    sub_model=args2.sub_model
    seed=args2.seed
    filename = filename_prompt + " [" + sub_model + "] " + str(seed)

    try:
        Image.open(DefaultPaths.output_path+'/'+filename+'.png')
        # Image.open(DefaultPaths.output_path+'/'+filename+'_batch 00001.png')
        from datetime import datetime
        now = datetime.now()
        dt_string = now.strftime("%m%d %H%M%S")
        filename = filename+'('+dt_string+')'
        print("Rename duplicate filename")
      ...
      """
        ...
        def run(self, x):
            ...
            try:
                ...
                save_filename = f"{DefaultPaths.output_path}/{filename}.png"
                # old_path = save_filename+'.png'
                # new_path = save_filename+'.png'
                # if old_path==new_path:
                #     new_path = save_filename+'('+str(random.randint(0, 1000))+').png'
                #     print("Rename")
                # shutil.move(old_path, new_path)
                file_list = []
                if path_exists(save_filename):
                    ...
                    if last_name[-15:-10] == "batch":
                        ...
                        count_string = f"{count_value:05d}"
                        save_filename = f"{DefaultPaths.output_path}/{filename}_batch {count_string}.png"
                    else:
                        save_filename = f"{DefaultPaths.output_path}/{filename}_batch 00001.png"
                shutil.copyfile(
                    args2.image_file,
                    save_filename,
                )
  ...
  """
    ...
    def save_settings():
        setting_list = {
            ...
            # "clip_model1": clip_model2,
            # "clip_model2": clip_model3,
            # "clip_model3": clip_model4,
            # "clip_model4": clip_model5,
    ...
    for pr in range(0, mtpl_prmpts_btchs):
        ...
        if generate_video:
            video_generation(args2, DefaultPaths,status,filename)